...
class CarrinhoServices extends BaseService<Carrinho> {
    constructor(
        @inject(CarrinhoRepository) private readonly carrinhoRepository: CarrinhoRepository
    ) {
        super(carrinhoRepository);
    }

    public async getCarrinhoByUsuarioId(usuarioId: number) {
        const carrinho = await this.carrinhoRepository.findByUsuarioId(usuarioId);
        if (!carrinho) throw new NotFoundError("Carrinho desse usuario não existe na Aplicação.");
        return carrinho;
    }
    ...
    public async getCarrinhoWithVariacoesByUsuarioId(usuarioId: number) {
      ...
      if (!carrinho) throw new Error("Carrinho não encontrado para esse usuario");
      return carrinho;
    }
    ...
    public async getCarrinhoWithVariacoes(carrinhoId: number) {
      ...
      if (!carrinho) throw new Error("Carrinho não encontrado para o usuário");
      return carrinho;
    }
    ...
    public async adicionarProdutoCarrinho(usuarioId: number, variacaoId: number): Promise<Carrinho> {
        ...
        if (!carrinhoComProduto) throw new BadRequestError("Dados de requisição invalidados");
