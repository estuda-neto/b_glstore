...
class NotificacaoController extends Controller {
  constructor(
    @inject(NotificacaoServices)
    private readonly notificacaoServices: NotificacaoServices,
  ...
  public async sendEmail(
    ...
    await Controller.tryCatch(
      async (req, res, next) => {
        ...
        const email = await this.emailServices.sendEmail(
          to,
          subject,
          text,
          html
  ...
  public async cadastrar(
    ...
    await Controller.tryCatch(
      async (req, res) => {
        const notificacao =
          await this.notificacaoServices.createWithAssociatons(req.body);
  ...
  public async atualizarnotificacao(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    await Controller.tryCatch(
      async (req, res, next) => {
        const { id } = req.params;
        const atualizado = await this.notificacaoServices.update(
          Number(id),
          req.body
        );
        if (!atualizado) {
          return next(
            new Error("notificacao não encontrado ou sem alterações")
          );
        }
  ...
  public async buscarPorUsuarioId(
    ...
    await Controller.tryCatch(
      async (req, res) => {
        const { id } = req.params;
        const notoficacoes = await this.notificacaoServices.buscarPorUsuarioId(
          Number(id)
        );
